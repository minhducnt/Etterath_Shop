...
import { ArrowRightSm, ArrowRightSmTwo } from '@svgs';
import { useGetProductTypeCategoryQuery } from '@/redux/features/categoryApi';
import { HomeThreeCategoryLoader } from '../loader';

const BeautyCategory = () => {
    const { data: categories, isLoading, isError } = useGetProductTypeCategoryQuery('beauty');
        ...
        content = category_items.map(item => (
            <div key={item._id} className="col-lg-3 col-sm-6">
                <div className="tp-category-item-3 p-relative black-bg text-center z-index-1 fix mb-30">
                    <div className="tp-category-thumb-3 include-bg" style={{ backgroundImage: `url(${item.img})` }}></div>
                    <div className="tp-category-content-3 transition-3">
                        ...
                        <span className="tp-categroy-ammount-3">{item.products.length} Products</span>
                        <div className="tp-category-btn-3">
                            <a onClick={() => handleCategoryRoute(item.parent)} className="cursor-pointer tp-link-btn tp-link-btn-2">
                                View Now
                                <ArrowRightSm />
                            </a>
                        </div>
