...
import ShopColorLoader from '@/global/components/loader/shop/color-filter-loader';
...
const ColorFilter = ({ setCurrPage, shop_right = false }) => {
    ...
    const handleColor = clr => {
        setCurrPage(1);
        router.push(`/${shop_right ? 'shop-right-sidebar' : 'shop'}?color=${clr.toLowerCase().replace('&', '').split(' ').join('-')}`);
    ...
    if (!isLoading && !isError && products?.data?.length > 0) {
        ...
        product_items.forEach(product => {
            let uniqueColor = new Set(product.image.map(item => item?.color));
                    ...
                    <li key={i}>
                        <div className="tp-shop-widget-checkbox-circle">
                            <input
                                ...
                                id={item.name}
                                checked={
                                    router.query.color === item.name.toLowerCase().replace('&', '').split(' ').join('-') ? 'checked' : false
                                }
                                readOnly
                            />
                            ...
                            <span style={{ backgroundColor: `${item.clrCode}` }} className="tp-shop-widget-checkbox-circle-self"></span>
                        </div>
