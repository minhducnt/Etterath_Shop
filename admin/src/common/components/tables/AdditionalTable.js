...
function AdditionalTable(props) {
  const { data, columns, handleSwitchStatus, tableRowAction, hideReset, hideDeleteRange, noPaging, hideAction } = props;
                          ...
                          >
                            {(cell => {
                              if (cell?.column?.type === 'dateTime' && !isNaN(Date.parse(cell?.value))) {
                                ...
                                return (
                                  <Flex>
                                    <ItemWithPreview alt="item" altBoxSize="56px" boxSize="56px" src={cell?.value} />
                                  </Flex>
                                );
                                ...
                                if (cell?.value?.length === 1) {
                                  return (
                                    <Flex>
                                      <ItemWithPreview alt="item" altBoxSize="56px" boxSize="56px" src={cell?.value[0]} />
                                    </Flex>
                                  );
                                } else {
                                  return (
                                    <Flex>
                                      <ItemListWithPreview alt="items" altBoxSize="56px" boxSize="56px" src={cell?.value} />
                                    </Flex>
                                  );
                                }
                              ...
                              } else if (cell?.column?.id === 'payment_method_name') {
                                return cell?.render('Cell', {
                                  value: `${Helper.toSentence(cell?.value)}`
                                });
                              } else {
                                return cell?.render('Cell');
                              }
