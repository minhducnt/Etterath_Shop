...
func (c *offerDatabase) FindAllOfferProducts(ctx context.Context,
	...
	query := `SELECT op.id AS offer_product_id, op.product_id, p.name AS product_name, op.offer_id, o.name AS offer_name, o.discount_rate
	FROM offer_products op INNER JOIN products p ON p.id = op.product_id INNER JOIN offers o ON o.id = op.offer_id`
	err = c.DB.Raw(query).Scan(&offerProducts).Error

	return
}

// find all offer_products by offer_id
func (c *offerDatabase) FindAllOfferProductsByID(ctx context.Context, offerID uint) (offerProducts []responses.OfferProduct, err error) {

	query := `SELECT op.id AS offer_product_id, op.product_id, p.name AS product_name, p.discount_price, op.offer_id, o.name AS offer_name, o.discount_rate
	FROM offer_products op INNER JOIN products p ON p.id = op.product_id INNER JOIN offers o ON o.id = op.offer_id
	WHERE op.offer_id = $1`
	err = c.DB.Raw(query, offerID).Scan(&offerProducts).Error
...
func (c *offerDatabase) UpdateOfferProduct(ctx context.Context, productOfferID, offerID uint) error {
	...
	query := `UPDATE offer_products SET offer_id = $1 WHERE product_id = $2`
...
func (c *offerDatabase) UpdateProductItemsDiscountByCategoryOfferID(ctx context.Context,
	...
	query := `UPDATE products AS pi SET discount_price = (pi.price * (100 - o.discount_rate))/100 
	FROM offer_categories oc 
...
func (c *offerDatabase) RemoveProductItemsDiscountByCategoryOfferID(ctx context.Context,
	...
	query := `UPDATE products AS pi SET discount_price = 0 
	FROM offer_categories oc 
	INNER JOIN products p ON p.category_id = oc.category_id
	INNER JOIN offers o ON o.id = oc.offer_id 
	WHERE oc.id = $1`
...
func (c *offerDatabase) UpdateProductItemsDiscountByProductOfferID(ctx context.Context, productOfferID uint) error {
	...
	query := `UPDATE products pi SET discount_price = 0 
	FROM offer_products op
	INNER JOIN offers o ON o.id = op.offer_id  
	WHERE pi.id = op.product_id AND op.id = $1`
	err := c.DB.Exec(query, productOfferID).Error
