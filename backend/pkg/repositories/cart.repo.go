...
func (c *cartDatabase) SaveCartItem(ctx context.Context, cartId, productItemId uint, qty uint) error {

	query := `INSERT INTO cart_items (cart_id, product_item_id, qty) VALUES ($1, $2, $3)`
	err := c.DB.Exec(query, cartId, productItemId, qty).Error
...
func (c *cartDatabase) FindAllCartItemsByCartID(ctx context.Context, cartID uint) (cartItems []responses.CartItem, err error) {
	...
	query := `SELECT ci.product_item_id, p.name AS product_name, ci.qty, p.price, p.discount_price, pi.qty_in_stock,
		CASE WHEN p.discount_price > 0 THEN p.discount_price * ci.qty ELSE p.price * ci.qty END AS sub_total
		FROM cart_items ci
		INNER JOIN product_items pi ON ci.product_item_id = pi.id
