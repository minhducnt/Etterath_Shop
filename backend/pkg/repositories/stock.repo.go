...
func (c *stockDatabase) Update(ctx context.Context, valuesToUpdate requests.UpdateStock) error {
	query := `UPDATE product_items SET qty_in_stock = qty_in_stock + $1, qty_out_stock = qty_out_stock + $2 WHERE sku = $3`

	err := c.DB.Exec(query, valuesToUpdate.QtyToAdd, valuesToUpdate.QtyToRemove, valuesToUpdate.SKU).Error
...
func (c *stockDatabase) FindAll(ctx context.Context, pagination requests.Pagination) (stocks []responses.Stock, err error) {
	...
	query := `SELECT pi.id AS product_item_id, pi.sku, pi.qty_in_stock, pi.qty_out_stock,
	p.name AS product_name, p.price as price, p.puscharge_price, p.discount_price
