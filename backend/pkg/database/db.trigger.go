...
var (
	...
	IF (TG_OP = 'DELETE') THEN 
		UPDATE carts c 
			SET total_price = ( 
				SELECT COALESCE ( SUM ( CASE WHEN p.discount_price > 0 THEN p.discount_price * ci.qty ELSE p.price * ci.qty END), 0)::bigint 
				FROM cart_items ci INNER JOIN product_items pi ON ci.product_item_id = pi.id INNER JOIN products p ON pi.product_id = p.id
	...
	ELSE 
		UPDATE carts c 
			SET total_price = (
				SELECT SUM (CASE WHEN p.discount_price > 0 THEN p.discount_price * ci.qty ELSE p.price * ci.qty END) 
				FROM cart_items ci INNER JOIN product_items pi ON ci.product_item_id = pi.id INNER JOIN products p ON pi.product_id = p.id 
	...
	// for calling the trigger function above when an event of insert or update or delete happen on cart_items
	...
	BEGIN 
		IF (TG_OP = 'INSERT') THEN 
			...
			SET qty_in_stock = pi.qty_in_stock - NEW.qty, qty_out_stock = pi.qty_out_stock + NEW.qty
	...
	//for order return time product_item quantity update
	...
	BEGIN
	  IF (TG_OP = 'UPDATE') THEN 
		EXECUTE format('UPDATE product_items pi
						SET qty_in_stock = qty_in_stock + ol.qty, , qty_out_stock = qty_out_stock + ol.qty
						FROM %I ol
