...
func (c *cartUseCase) SaveProductItemToCart(ctx context.Context, userID, productItemId, qty uint) error {
	...
	if cartItem.ID != 0 {
		...
		if err := c.cartRepo.UpdateCartItemQty(ctx, cartItem.ID, newQty); err != nil {
			return utils.PrependMessageToError(err, "failed to update cart item qty")
		}
	} else {
		// add productItem to cartItem
		err = c.cartRepo.SaveCartItem(ctx, cart.ID, productItemId, qty)
		if err != nil {
