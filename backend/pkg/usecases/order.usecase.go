...
func NewOrderUseCase(orderRepo interfaces.OrderRepository, cartRepo interfaces.CartRepository,
	userRepo interfaces.UserRepository,
	paymentRepo interfaces.PaymentRepository, cloudService cloud.CloudService) service.OrderUseCase {
	return &OrderUseCase{
		orderRepo:    orderRepo,
		cartRepo:     cartRepo,
		userRepo:     userRepo,
...
func (c *OrderUseCase) FindOrderItems(ctx context.Context, shopOrderID uint,
	...
	if len(orderItems) == 0 {
		return orderItems, nil // No items to process, return early
	}
	...
	go func() {
		for i := range orderItems {
			...
			case <-newCtx.Done():
				return
			default:
				url, err := c.cloudService.GetFileURLFromFisebaseService(newCtx, orderItems[i].Image)
				if err != nil {
					errChan <- utils.PrependMessageToError(err, "failed to get image url from cloud service")
					return
				}
				orderItems[i].Image = url
			}
		}
		errChan <- nil
	}()
