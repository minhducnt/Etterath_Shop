...
import (
	...
	repo "etterath_shop_feature/pkg/repositories/interfaces"
...
func (c *offerUseCase) SaveProductOffer(ctx context.Context, offerProduct models.OfferProduct) error {
	...
	isExist, err := c.offerRepo.FindOfferProductByProductID(ctx, offerProduct.ProductID)
	if err != nil {
...
// get all offers for products by offer id
func (c *offerUseCase) FindAllProductOffersByID(ctx context.Context, offerID uint) ([]responses.OfferProduct, error) {
	productOffers, err := c.offerRepo.FindAllOfferProductsByID(ctx, offerID)
	if err != nil {
		return nil, utils.PrependMessageToError(err, "failed to find product offers by offer id")
	}
